...
if __name__ == '__main__':
    ...
    for year_range in year_tuple:
        ...
        if year_range[0] == "2014-01-01":
            all_codes = cashflow_statement.universe('20140430')
        else:
            all_codes = cashflow_statement.universe('20170430')  # 20170430
        date_range = cashflow_statement.calendar(year_range[0], year_range[1])
        N = len(all_codes)
        n = 0
        while n < N:
            ...
            try:
                ...
                # factor_values, _, _ = balance_sheet.raw(codes, date_range, 'total_asset', factor_method='ttm')
                # values = IncomeTs().ttm('net_income2major', codes[:100], min(date_range), max(date_range))
