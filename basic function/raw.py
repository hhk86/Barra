...
if __name__ == '__main__':
    ...
    codes = income_statement.universe('20190701')
    date_range = income_statement.calendar('20090101', '20190830')
    ...
    factor_values, snapshots, _, _ = income_statement.raw(["600626.SH", ], date_range, 'total_revenue', factor_method='ttm')
    # 获取天软数据，用于对比（黄泓恺已对比）
    ...
    factor_values, snapshots, _, _ = balance_sheet.raw(codes[:20], date_range, 'long_term_debt', factor_method='avg')
    # 获取天软数据，用于对比（黄泓恺已对比）
    ...
    # 现金流量表的原始值，已考虑了ttm，以及对后续数据更正的调整
    factor_values, snapshots, _, _ = cashflow_statement.raw(codes[:20], date_range, 'operating_cashflow', factor_method='ttm')
    # 获取天软数据，用于对比（黄泓恺已对比）
